...
export default function PropertyDetails() {
  ...
  if (isLoading) {
    return (
      <div className="py-24 text-center">
        ...
        <p className="text-slate-500 font-bold text-sm">Loading complete property information...</p>
      </div>
    );
  }
  ...
  const ownerPhone = property.contactPhone || property.ownerId?.phone || "+91 98765 43210";
  ...
  const backLink = (session?.user as any)?.role === "owner" ? "/owner/properties" : "/properties";
  const backLabel = (session?.user as any)?.role === "owner" ? "Back to My Properties" : "Back to Properties";
  ...
  return (
    <div className="p-4 sm:p-8 max-w-7xl mx-auto space-y-6">
      {/* Owner Live View Preview Notice */}
      {isOwner && (
        <div className="bg-indigo-50 border border-indigo-200 text-indigo-900 p-4 rounded-3xl flex flex-col sm:flex-row items-center justify-between gap-3 shadow-xs">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-xs sm:text-sm font-black text-indigo-950">
                Owner Live View Mode
              </p>
              <p className="text-[11px] sm:text-xs text-indigo-700 font-medium">
                You are viewing this listing in the exact view shown to prospective tenants.
              </p>
            </div>
          </div>
          <Link
            href="/owner/properties"
            className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl shadow-xs transition-all active:scale-95 shrink-0"
          >
      ...
      {property.status && property.status !== "Active" && (
        <div className="bg-amber-100 border border-amber-300 text-amber-900 px-4 py-3 rounded-2xl text-xs font-bold flex items-center gap-2.5">
          ...
          <span>
            Listing Status: &quot;{property.status}&quot;. This listing will become live to public tenants once approved by the Provider App admin team.
          </span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-2 border rounded-xl font-semibold text-xs shadow-xs transition-all cursor-pointer ${isSaved
              ...
              <div className="flex items-center gap-2 bg-black/60 backdrop-blur-md p-1.5 rounded-2xl border border-white/20 pointer-events-auto">
                {images.slice(0, 5).map((img: string, i: number) => (
                  <button
                    ...
                    className={`relative w-12 h-10 sm:w-14 sm:h-11 rounded-xl overflow-hidden border-2 transition-all cursor-pointer ${selectedImgIdx === i ? "border-blue-500 scale-105 ring-2 ring-blue-400/50" : "border-white/40 opacity-75 hover:opacity-100"
                      }`}
                  >
                    {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-gradient-to-br from-indigo-50/80 via-white to-blue-50/50 p-6 sm:p-8 rounded-3xl border border-indigo-200/80 shadow-sm space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-indigo-100 pb-4">
                ...
                <div className="flex items-center bg-white p-1 rounded-2xl border border-indigo-200 shadow-2xs self-start sm:self-auto">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all cursor-pointer ${coolingPreference === "ac"
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-extrabold transition-all cursor-pointer ${coolingPreference === "nonAc"
                ...
                {roomPricingsList.map((tier: any) => {
                  ...
                  return (
                    <button
                      ...
                      className={`p-4 rounded-2xl text-left border-2 transition-all cursor-pointer flex flex-col justify-between space-y-2 ${isSelected
              ...
              <div className="p-4 bg-white rounded-2xl border border-indigo-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs font-semibold text-slate-700">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    Currently Selected: <strong className="text-indigo-900">{priceNote || selectedSeater}</strong> at{" "}
          ...
          {/* Room Seater & AC / Non-AC Rates Breakdown Table */}
          {roomPricingsList.length > 0 && (
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm space-y-4">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-xl shadow-blue-900/5 sticky top-8 space-y-6">
                ...
                <div className="flex items-center justify-between text-xs font-bold text-slate-700 pt-1 border-t border-slate-200/60">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      type="button"
                      onClick={() => setCoolingPreference("ac")}
                      className={`px-2.5 py-0.5 rounded text-[11px] font-bold ${coolingPreference === "ac" ? "bg-blue-600 text-white" : "bg-white border text-slate-600"
                        }`}
                    >
                      AC
                    </button>
                    <button
                      type="button"
                      onClick={() => setCoolingPreference("nonAc")}
                      className={`px-2.5 py-0.5 rounded text-[11px] font-bold ${coolingPreference === "nonAc" ? "bg-slate-800 text-white" : "bg-white border text-slate-600"
                        }`}
                    >
                      Non-AC
                    </button>
            ...
            {hasContacted ? (
              <div className="space-y-3 p-4 bg-emerald-50 border border-emerald-200 rounded-2xl text-center">
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                  ...
                  <a
                    href={`https://wa.me/${ownerPhone.replace(/\D/g, "")}?text=Hi%2C%20I%20am%20interested%20in%20your%20property%20listing%3A%20${encodeURIComponent(property.title)}`}
                    target="_blank"
                    rel="noreferrer"
                    className="block w-full py-2.5 bg-green-600 hover:bg-green-700 text-white text-xs font-bold rounded-xl shadow-sm transition-all"
                  >
                    WhatsApp
                  </a>
                </div>
              ...
              <div className="space-y-2.5">
                ...
                {hasRequestedAssistance ? (
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-extrabold rounded-2xl border border-slate-200/80 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                  >
            ...
            <p className="text-center text-[11px] text-slate-400 leading-relaxed">
               verified listing. No brokerage fees or hidden charges.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-center gap-2 overflow-x-auto py-2 z-10 max-w-full">
            {images.map((img: string, i: number) => (
              <button
                ...
                className={`relative w-14 h-12 sm:w-18 sm:h-14 rounded-xl overflow-hidden border-2 transition-all shrink-0 cursor-pointer ${lightboxImgIdx === i ? "border-blue-500 scale-105 ring-2 ring-blue-400" : "border-white/30 opacity-60 hover:opacity-100"
                  }`}
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
